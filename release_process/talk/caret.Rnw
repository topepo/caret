...
\title{Nobody Knows What It's Like To Be the Bad Man --- The Development Process for the {\tt caret} Package}
...
a little bit about travis and coveralls
...
\item Coveralls notifies to write tests for new functions
\item Iterate several times
\end{itemize}
