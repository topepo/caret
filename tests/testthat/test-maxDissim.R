...
test_that("maxDissim can return row names instead of indices", {
  expect_equal(maxDissim(maxdiss_base, maxdiss_pool, n = 1, useNames = TRUE), "p3")
})
...
test_that("maxDissim supports alternative objective functions", {
  ...
  expect_equal(maxDissim(maxdiss_base, maxdiss_pool, n = 2, obj = sumDiss), c(3, 1))
})
...
test_that("maxDissim can subsample candidates with randomFrac", {
  ...
  expect_length(res, 2)
  expect_true(all(res %in% seq_len(nrow(maxdiss_pool))))
})

test_that("maxDissim prints progress when verbose", {
  expect_output(maxDissim(maxdiss_base, maxdiss_pool, n = 1, verbose = TRUE), "adding")
})
...
test_that("maxDissim validates its arguments", {
  ...
  expect_error(maxDissim(maxdiss_base, maxdiss_pool, n = 99), "n must be less than")
  expect_error(maxDissim(maxdiss_base, maxdiss_pool, randomFrac = 2), "randomFrac must be in")
  expect_error(maxDissim(maxdiss_base, maxdiss_pool, randomFrac = 0), "randomFrac must be in")
})
...
test_that("splitByDissim returns a subsample of row indices", {
  ...
  expect_true(all(res %in% seq_len(nrow(maxdiss_split_x))))
})
