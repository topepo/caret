...
## The modelling packages used below are all suggested, not required. When any
## of them is missing the chunks are left unevaluated so that the vignette
## still builds (e.g. in a check that installs hard dependencies only).
needed <- c("MASS", "mlbench", "pls", "klaR")
has_pkgs <- all(vapply(needed, requireNamespace, logical(1), quietly = TRUE))
...
## `eval` does not apply to inline code, so inline results go through this
inl <- function(x) tryCatch(x, error = function(e) "")
...
The Sonar data consist of `r inl(nrow(Sonar))` data points collected on `r inl(ncol(Sonar)-1)` predictors. The goal is to predict the two classes `M` for metal cylinder or `R` for rock).
...
Based on this analysis, the difference between the models is `r inl(round(diffs$statistics$ROC[[1]]$estimate, 3))` ROC units (the RDA model is slightly higher) and the two-sided _p_-value for this difference is `r inl(format.pval(diffs$statistics$ROC[[1]]$p.value))`.
