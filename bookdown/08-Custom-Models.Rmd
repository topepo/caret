...
library(knitr)
opts_chunk$set(comment="   ", digits = 3, tidy = FALSE,background="#FFFF00")
library(caTools)
library(klaR)
library(mlbench)
library(kernlab)
...
```{r custom_reset_workers, echo=FALSE, results = 'hide'}
registerDoSEQ()
``` 
...
ggplot(results, aes(x = nIter, y = SpeedUp)) + 
  ...
  ylab("Speed-Up")
```
```{r custom_reset_workers2, echo=FALSE, results = 'hide'}
...
set.seed(184)
plsrf <- train(x = absorpTrain, y = proteinTrain, 
               method = pls_rf,
               preProc = c("center", "scale"),
               tuneLength = 10,
               ntree = 1000,
