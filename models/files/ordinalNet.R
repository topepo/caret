modelInfo <- list(label = "Penalized Ordinal Regression",
                  ...
                  type = "Classification",
                  parameters = data.frame(parameters = c('alpha', 'lambda', 'modeltype', 'family'), class = c('numeric', 'numeric', 'character', 'character'), 
                  ...
                  predict = function(modelFit, newdata, submodels = NULL) {if (!is.matrix(newdata)) newdata <- as.matrix(newdata)
                                                                           out <- modelFit$obsLevels[predict(modelFit, newdata, type = 'class')]
                                                                           out 
                                                                          },
                  prob = function(modelFit, newdata, submodels = NULL) {if (!is.matrix(newdata)) newdata <- as.matrix(newdata)
                  ...
                  sort = function(x) {model_type_order <- c('parallel' = 1, 'semiparallel' = 2, 'nonparallel' = 3)
