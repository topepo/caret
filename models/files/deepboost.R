modelInfo <- list(label = "DeepBoost",
                  ...
                  loop = NULL,
                  type = 'Classification',
                  parameters = data.frame(parameter = c('num_iter', "tree_depth", 'beta', 'lambda', "loss_type"),
                  ...
                  grid = function(x, y, len = NULL, search = "grid") {
                    if(search == "grid") {
                      out <- expand.grid(tree_depth = seq(1, len),
                                         num_iter = floor((1:len) * 50),
                                         beta = 2^seq(-8, -4, length = len),
                                         lambda = 2^seq(-6, -2, length = len),
                                         loss_type = "l")
                    } else {
                      out <- data.frame(num_iter = floor(runif(len, min = 1, max = 500)),
