...
#' al. (2008).
#'
#' The results from [train()] can have more than one performance metric per
#' resample. Each metric in the input object is saved.
#'
#' `resamples` checks that the resampling results match; that is, the indices
#' in the object `trainObject$control$index` are the same. Also, the argument
#' [trainControl()] `returnResamp` should have a value of `"final"` for each
#' model.
#'
#' The summary function computes summary statistics across each model/metric
#' combination.
#'
#' @aliases resamples.default resamples summary.resamples sort.resamples as.matrix.resamples as.data.frame.resamples modelCor
#' @param x a list of two or more objects of class [train()], [sbf()] or
#'   [rfe()] with a common set of resampling indices in the `control` object.
#'   For `sort.resamples`, it is an object generated by `resamples`.
#' @param modelNames an optional set of names to give to the resampling results
#' @param object an object generated by `resamples`
...
#'   pass to `sort` or `FUN`.
#' @return For `resamples`: an object with class `"resamples"` with elements
#' * `call`: the call
#' * `values`: a data frame of results where rows correspond to resampled data
#'             sets and columns indicate the model and metric
#' * `models`: a character string of model labels
#' * `metrics`: a character string of performance metrics
#' * `methods`: a character string of the [train()] `method` argument values
#'              for each model
#'
#' For `sort.resamples` a character string in the sorted order is generated.
#' `modelCor` returns a correlation matrix.
#' @author Max Kuhn
#' @seealso [train()], [trainControl()], [diff.resamples()],
#'   [xyplot.resamples()], [densityplot.resamples()], [bwplot.resamples()],
#'   [splom.resamples()]
...
resamples.default <- function(x, modelNames = names(x), ...) {
  ...
  if (any(rs_method == "LOOCV")) {
    stop(
      "LOOCV is not compatible with `resamples()` since only one resampling ",
  ...
  for (i in seq(along.with = x)) {
    if (class(x[[i]])[1] == "rfe" && x[[i]]$control$returnResamp == "all") {
      warning(paste0(
        "'",
        modelNames[i],
        "' did not have 'returnResamp=\"final\"; the optimal subset is used"
      ))
    }
    if (class(x[[i]])[1] == "train" && x[[i]]$control$returnResamp == "all") {
      warning(paste0(
        "'",
        modelNames[i],
        "' did not have 'returnResamp=\"final\"; the optimal tuning parameters are used"
      ))
    }
  ...
  all_names <- lapply(rs_values, function(x) names(x)[names(x) != "Resample"])
  ...
  if (any(all_names < length(x))) {
    warning(paste(
      "Some performance measures were not computed for each model:",
...
summary.resamples <- function(object, metric = object$metrics, ...) {
  ...
  for (i in seq(along.with = metric)) {
    tmpData <- vals[,
      grep(paste("~", metric[i], sep = ""), names(vals), fixed = TRUE),
      drop = FALSE
    ]

    out[[i]] <- do.call("rbind", lapply(tmpData, function(x) summary(x)[1:6]))
    naSum <- matrix(
      unlist(lapply(tmpData, function(x) sum(is.na(x)))),
      ncol = 1
    )
    colnames(naSum) <- "NA's"
...
as.matrix.resamples <- function(x, metric = x$metric[1], ...) {
  ...
  if (!(any(get_cols))) {
    stop("no columns fit that metric")
  }
...
#' @rdname resamples
#' @export
as.data.frame.resamples <- function(
  x,
  row.names = NULL,
  optional = FALSE,
  metric = x$metric[1],
...
#' and returns the results as an object with classes `prcomp.resamples` and
#' `prcomp`.
#'
...
#' [stats::prcomp()] can be used, although custom `print` and `plot` methods
#' are used.
#'
#' The plot method uses lattice graphics. When `what = "scree"` or `what =
#' "cumulative"`, [lattice::barchart()] is used. When `what = "loadings"` or
#' `what = "components"`, either [lattice::xyplot()] or [lattice::splom()] are
#' used (the latter when `dims` > 2). Options can be passed to these methods
#' using `...`.
#'
#' When `what = "loadings"` or `what = "components"`, the plots are put on a
#' common scale so that later components are less likely to be
#' over-interpreted. See Geladi et al. (2003) for examples of why this can be
#' important.
#'
#' For clustering, [stats::hclust()] is used to determine clusters of models
#' based on the resampled performance values.
#'
#' @aliases prcomp.resamples cluster.resamples cluster plot.prcomp.resamples
#' @param x For `prcomp`, an object of class [resamples()] and for
#'   `plot.prcomp.resamples`, an object of class `plot.prcomp.resamples`
#' @param metric a performance metric that was estimated for every resample
#' @param what the type of plot: `"scree"` produces a bar chart of standard
#'   deviations, `"cumulative"` produces a bar chart of the cumulative percent
#'   of variance, `"loadings"` produces a scatterplot matrix of the loading
#'   values and `"components"` produces a scatterplot matrix of the PCA
#'   components
#' @param dims The number of dimensions to plot when `what = "loadings"` or
#'   `what = "components"`
#' @param \dots For `prcomp.resamples`, options to pass to [stats::prcomp()],
#'   for `plot.prcomp.resamples`, options to pass to Lattice objects (see
#'   Details below) and, for `cluster.resamples`, options to pass to `hclust`.
#' @return For `prcomp.resamples`, an object with classes `prcomp.resamples`
#'   and `prcomp`. This object is the same as the object produced by `prcomp`,
...
#' @author Max Kuhn
#' @seealso [resamples()], [lattice::barchart()], [lattice::xyplot()],
#'   [lattice::splom()], [stats::hclust()]
...
prcomp.resamples <- function(x, metric = x$metrics[1], ...) {
  ...
  tmpData <- x$values[,
    grep(paste("~", metric, sep = ""), names(x$value), fixed = TRUE),
    drop = FALSE
  ]
  names(tmpData) <- gsub(
    paste("~", metric, sep = ""),
...
plot.prcomp.resamples <- function(
  x,
  what = "scree",
  dims = max(2, ncol(x$rotation)),
  ...
) {
  if (length(what) > 1) {
    stop("one plot at a time please")
  }
  switch(
    what,
    scree = {
    ...
    cumulative = {
      barchart(
        cumsum(x$sdev^2) / sum(x$sdev^2) ~ paste(
          "PC",
          gsub(" ", "0", format(seq(along.with = x$sdev))),
          sep = ""
        ),
...
#' @export
print.prcomp.resamples <- function(
  x,
...
print.resamples <- function(x, ...) {
  ...
  if (any(hasTimes)) {
    ...
    cat(
      "Time estimates for:",
      paste(tolower(names(hasTimes))[hasTimes], collapse = ", "),
      "\n"
    )
  }
  invisible(x)
...
#'
#' Lattice and ggplot functions for visualizing resampling results across
#' models
#'
#' The ideas and methods here are based on Hothorn et al. (2005) and Eugster et
#' al. (2008).
...
#'
#' `densityplot` and `bwplot` display univariate visualizations of the
#' resampling distributions while `splom` shows the pair-wise relationships.
#'
#' @aliases xyplot.resamples densityplot.resamples bwplot.resamples splom.resamples parallelplot.resamples dotplot.resamples ggplot.resamples
#' @param x an object generated by `resamples`
#' @param data Only used for the `ggplot` method; an object generated by
#'   `resamples`
#' @param models a character string for which models to plot. Note: `xyplot`
#'   requires one or two models whereas the other methods can plot more than
#'   two.
#' @param metric a character string for which metrics to use as conditioning
#'   variables in the plot. `splom` requires exactly one metric when `variables
#'   = "models"` and at least two when `variables = "metrics"`.
#' @param variables either "models" or "metrics"; which variable should be
#'   treated as the scatter plot variables?
#' @param panelRange a common range for the panels. If `NULL`, the panel ranges
#'   are derived from the values across all the models
#' @param what for `xyplot`, the type of plot. Valid options are: "scatter"
#'   (for a plot of the resampled results between two models), "BlandAltman" (a
#'   Bland-Altman, aka MA plot between two models), "tTime" (for the total time
#'   to run `train` versus the metric), "mTime" (for the time to build the
#'   final model) or "pTime" (the time to predict samples - see the
#'   `timingSamps` options in [trainControl()], [rfeControl()], or
#'   [sbfControl()])
#' @param units either "sec", "min" or "hour"; which `what` is either "tTime",
#'   "mTime" or "pTime", how should the timings be scaled?
#' @param conf.level the confidence level for intervals about the mean
#'   (obtained using [stats::t.test()])
#' @param \dots further arguments to pass to either [lattice::histogram()],
#'   [lattice::densityplot()], [lattice::xyplot()], [lattice::dotplot()] or
#'   [lattice::splom()]
#' @return a lattice object
#' @author Max Kuhn
#' @seealso [resamples()], [lattice::dotplot()], [lattice::bwplot()],
...
xyplot.resamples <- function(
  ...
  if (!(units %in% c("min", "sec", "hour"))) {
    stop("units should be 'sec', 'min' or 'hour'")
  }
  if (!(what %in% c("scatter", "BlandAltman", "tTime", "mTime", "pTime"))) {
    stop(
  ...
  if (what %in% c("tTime", "mTime", "pTime")) {
    ...
    prepanel.ci <- function(x, y, lx, ux, subscripts, ...) {
      ...
      list(xlim = extendrange(c(x, ux, lx)), ylim = extendrange(y))
    }
    ...
    tmpData <- apply(x$values[, -1], 2, function(x) {
      tmp <- t.test(x)
    ...
    times <- data.frame(Time = tm, Model = rownames(x$timings))
    plotData <- merge(times, tmpData)

    if (units == "min") {
      plotData$Time <- plotData$Time / 60
    }
    if (units == "hour") {
      plotData$Time <- plotData$Time / 60 / 60
    }
...
splom.resamples <- function(
  ...
  if (variables == "models") {
    ...
    if (is.null(panelRange)) {
      panelRange <- extendrange(tmpData)
    }
    splom(
      ~tmpData,
      panel = function(x, y) {
    ...
    if (variables == "metrics") {
      ...
      if (length(metric) < 2) {
        stop("There should be at least two metrics")
      }
      ...
      stop("'variables' should be either 'models' or 'metrics'")
    }
  }
}
...
bwplot.resamples <- function(
  ...
  plotData$Metric <- unlist(lapply(tmp, function(x) x[2]))
  plotData <- subset(plotData, Model %in% models & Metric %in% metric)
  avPerf <- ddply(subset(plotData, Metric == metric[1]), .(Model), function(x) {
    c(Median = median(x$value, na.rm = TRUE))
  })
...
dotplot.resamples <- function(
    ...
    function(x, cl) {
      ...
      if (class(ttest)[1] == "htest") {
        ...
        out <- c(
          LowerLimit = NA_real_,
          UpperLimit = NA_real_,
          Estimate = mean(x$value, na.rm = TRUE)
        )
      }
      out
    },
  ...
  Var2 <- NULL
  avPerf <- ddply(
    subset(results, Metric == metric[1] & Var2 == "Estimate"),
    .(Model),
  ...
  dotplot(
    ...
    panel = function(x, y) {
      ...
      y <- as.numeric(y)
      vals <- aggregate(x, list(group = y), function(x) {
        c(min = min(x), mid = median(x), max = max(x))
      })
...
#' `confLevel` to `1-((1-confLevel)/p)` here `p` is the number of pair-wise
#' comparisons are being made. For other correction methods, no such change is
#' used.
#'
#' `compare_models` is a shorthand function to compare two models using a
#' single metric. It returns the results of [stats::t.test()] on the
#' differences.
#'
...
#'   should have scalar outputs called `estimate` and `p.value`
#' @param object a object generated by `diff.resamples`
#' @param adjustment any p-value adjustment method to pass to
#'   [stats::p.adjust()].
#' @param confLevel confidence level to use for [dotplot.diff.resamples()]. See
#'   Details below.
#' @param digits the number of significant differences to display when printing
#' @param a,b two objects of class [train()], [sbf()] or [rfe()] with a common
#'   set of resampling indices in the `control` object.
#' @param \dots further arguments to pass to `test`
#' @return An object of class `"diff.resamples"` with elements:
#'
#' * `call`: the call
#' * `difs`: a list for each metric being compared. Each list contains a matrix
#'           with differences in columns and resamples in rows
#' * `statistics`: a list of results generated by `test`
#' * `adjustment`: the p-value adjustment used
#' * `models`: a character string for which models were compared.
#' * `metrics`: a character string of performance metrics that were used
#' * `call`: the call
#' * `table`: a list of tables that show the differences and p-values
#'
#' ...or (for `compare_models`) an object of class `htest` resulting from
#' [stats::t.test()].
#' @author Max Kuhn
#' @seealso [resamples()], [dotplot.diff.resamples()],
...
densityplot.diff.resamples <- function(x, data, metric = x$metric, ...) {
  plotData <- lapply(x$difs, function(x) {
    stack(as.data.frame(x, stringsAsFactors = TRUE))
  })
...
summary.diff.resamples <- function(
  ...
  for (h in seq(along.with = object$metric)) {
    ...
    pvals[lower.tri(pvals)] <- p.adjust(
      pvals[lower.tri(pvals)],
      method = object$adjustment
    )
    asText <- matrix(
      "",
      nrow = length(object$models),
    ...
    pvals2 <- matrix(
      format.pval(pvals, digits = digits),
      nrow = length(object$models)
    )
...
#'
#' `densityplot` and `bwplot` display univariate visualizations of the
#' resampling distributions. `levelplot` displays the matrix of pair-wide
#' comparisons. `dotplot` shows the differences along with their associated
#' confidence intervals.
#'
#' @aliases levelplot.diff.resamples densityplot.diff.resamples bwplot.diff.resamples dotplot.diff.resamples
#' @param x an object generated by [diff.resamples()]
#' @param data Not used
#' @param metric a character string for which metrics to plot. Note: `dotplot`
#'   and `levelplot` require exactly two models whereas the other methods can
#'   plot more than two.
#' @param \dots further arguments to pass to either [lattice::densityplot()],
#'   [lattice::dotplot()] or [lattice::levelplot()]
#' @return a lattice object
#' @author Max Kuhn
#' @seealso [resamples()], [diff.resamples()], [lattice::bwplot()],
#'   [lattice::densityplot()], [lattice::xyplot()], [lattice::splom()]
#' @keywords hplot
#' @examplesIf !caret:::is_cran_check()
#'
...
dotplot.diff.resamples <- function(x, data = NULL, metric = x$metric[1], ...) {
  ...
  plotData <- as.data.frame(
    matrix(NA, ncol = 3, nrow = ncol(x$difs[[metric]])),
  ...
  plotData$Difference <- gsub(
    ".diff.",
    " - ",
    colnames(x$difs[[metric]]),
    fixed = TRUE
  )
