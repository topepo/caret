...
ga_wrapper <- function(
  ...
  list(
    internal = c(internal, .indiv = indiv),
...
ga_select <- function(
  ...
  if (popSize > ng) {
    Pop[(ng + 1):popSize, ] <- funcs$initial(vars = nvars, popSize = popSize)[
      1:(popSize - ng),
    ]
  }
  ...
  for (generation in 1:iters) {
    ...
    if (!is.null(testX)) {
      best_external <- current_ext[best_index, ]
    }
    subsets[[generation]] <- which(Pop[best_index, ] == 1)
    internal$Size[generation] <- sum(Pop[best_index, ] == 1)

    if (generation > 1) {
      hist_best <- which.max(internal[
        1:(generation - 1),
        ga_metric["internal"]
      ])
      internal$Similarity[generation] <- jack_sim(
        index2vec(subsets[[hist_best]], ncol(Pop)),
        index2vec(subsets[[generation]], ncol(Pop))
      )
      tmp_sim <- apply(
        Pop,
        1,
        function(x, y) jack_sim(x, y),
    ...
    if (generation == 1) {
      ...
      if (!is.null(testX)) {
        for (new_var in names(best_external)) {
          external[, new_var] <- NA
        }
...
print.gafs <- function(
  ...
  if (is.function(x$ga_param$pmutation)) {
    ...
    cat("Mutation probability:", x$ga_param$pmutation, "\n")
  }
...
"gafs.default" <-
  function(
    ...
    if (is.null(gafsControl$maximize)) {
      gafsControl$maximize <- rep(
        ifelse(
          gafsControl$metric %in% c("RMSE", "MAE", "logLoss"),
          FALSE,
          TRUE
        ),
        2
    ...
    if (is.null(gafsControl$index)) {
      gafsControl$index <- switch(
        ...
        boot = ,
        boot632 = createResample(y, gafsControl$number),
    ...
    if (is.null(names(gafsControl$index))) {
      names(gafsControl$index) <- getFromNamespace("prettySeq", "caret")(
        gafsControl$index
      )
    }
...
update.gafs <- function(object, iter, x, y, ...) {
  ...
  if (!is.null(object$recipe)) {
    ...
    args <- list(
      x = x[, object$ga$subsets[[iter]], drop = FALSE],
      y = y,
      lev = object$levels,
      last = TRUE
    )
  ...
  if (object$control$verbose) {
    cat(
      ...
      length(object$ga$subsets[[iter]]),
      "predictors from generation",
      iter,
      "\n"
    )
  }
