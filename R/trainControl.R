...
#' required. This may not be obvious as `train` does some optimizations for
#' certain models. For example, when tuning over PLS model, the only model that
#' is fit is the one with the largest number of components. So if the model is
#' being tuned over `comp in 1:10`, the only model fit is `ncomp = 10`.
#' However, if the vector of integers used in the `seeds` arguments is longer
#' than actually needed, no error is thrown.
#'
#' Using `method = "none"` and specifying more than one model in [train()]'s
#' `tuneGrid` or `tuneLength` arguments will result in an error.
#'
#' Using adaptive resampling when `method` is either `"adaptive_cv"`,
#' `"adaptive_boot"` or `"adaptive_LGOCV"`, the full set of resamples is not
#' run for each model. As resampling continues, a futility analysis is
...
#' 2012). See <http://topepo.github.io/caret/random-hyperparameter-search.html>
...
#'
#' Note that if `index` or `indexOut` are specified, the label shown by `train`
#' may not be accurate since these arguments supersede the `method` argument.
#'
#' @param method The resampling method: `"boot"`, `"boot632"`,
#'   `"optimism_boot"`, `"boot_all"`, `"cv"`, `"repeatedcv"`, `"LOOCV"`,
#'   `"LGOCV"` (for repeated training/test splits), `"none"` (only fits one
#'   model to the entire training set), `"oob"` (only for random forest, bagged
#'   trees, bagged earth, bagged flexible discriminant analysis, or conditional
#'   tree forest models), `"timeslice"`, `"adaptive_cv"`, `"adaptive_boot"`, or
#'   `"adaptive_LGOCV"`. Not all methods are available for all control
...
#'   for each resample should be saved. Values can be either `"all"`,
#'   `"final"`, or `"none"`. A logical value can also be used that convert to
#'   `"all"` (for true) or `"none"` (for false). `"final"` saves the
#'   predictions for the optimal tuning parameters.
#' @param p For leave-group out cross-validation: the training percentage
#' @param search Either `"grid"` or `"random"`, describing how the tuning
#'   parameter grid is determined. See details below.
#' @param initialWindow,horizon,fixedWindow,skip possible arguments to
#'   [createTimeSlices()] when method is `timeslice`.
...
#'   [defaultSummary()]. Note that if `method = "oob"` is used, this option is
...
#' @param preProcOptions A list of options to pass to [preProcess()]. The type
#'   of pre-processing (e.g. center, scaling etc) is passed in via the
#'   `preProc` option in [train()].
...
#'   imbalances). Values are `"none"`, `"down"`, `"up"`, `"smote"`, or
#'   `"rose"`. The latter two values require the \pkg{themis} and \pkg{ROSE}
#'   packages, respectively. This argument can also be a list to facilitate
#'   custom sampling and these details can be found on the \pkg{caret} package
...
#' @param indexOut a list (the same length as `index`) that dictates which data
...
#'   are used to fit the final model after resampling. If `NULL`, then entire
#'   data set is used.
...
#'   only). If logical, the predictions can be constrained to be within the
#'   limit of the training set outcomes. For example, a value of `c(TRUE,
#'   FALSE)` would only constrain the lower end of predictions. If numeric,
#'   specific bounds can be used. For example, if `c(10, NA)`, values below 10
#'   would be predicted as 10 (with no constraint in the upper side).
...
#'   set of integers. Alternatively, a list can be used. The list should have
#'   `B+1` elements where `B` is the number of resamples. The first `B`
#'   elements of the list should be vectors of integers of length `M` where `M`
#'   is the number of models or subsets being evaluated. The last element of
#'   the list only needs to be a single integer (for the final model). See the
#'   Examples section below and the Details section.
#' @param adaptive a list used when `method` is `"adaptive_cv"`,
#'   `"adaptive_boot"` or `"adaptive_LGOCV"`. See Details below.
#' @param trim a logical. If `TRUE` the final model in `object\$finalModel` may
#'   have some components of the object removed so reduce the size of the saved
#'   object. The `predict` method will still work, but some other features of
#'   the model may not work. `trim`ing will occur only for models where this
...
#' Models'' <https://arxiv.org/abs/1405.6974>,
#'
