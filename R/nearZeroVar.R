...
#' looks at the distribution of the columns of `x` conditioned on the levels of
...
#' `freqCut`. Secondly, the ``percent of unique values,'' the number of unique
#' values divided by the total number of samples (times 100), must also be
#' below `uniqueCut`.
#'
#' In the above example, the frequency ratio is 999 and the unique value
#' percentage is 0.0001.
#'
#' Checking the conditional distribution of `x` may be needed for some models,
#' such as naive Bayes where the conditional distributions should have at least
#' one data point within a class.
#'
...
#'   `x` for a given resample
#' @param foreach should the \pkg{foreach} package be used for the
#'   computations? If `TRUE`, less memory should be used.
#' @param allowParallel should the parallel processing via the \pkg{foreach}
#'   package be used for the computations? If `TRUE`, more memory will be used
