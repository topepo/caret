...
#'
#' The first function (`twoClassSim`) generates two class data. The data are
#' simulated in different sets. First, two multivariate normal predictors
#' (denoted here as `A` and `B`) are created with a correlation our about 0.65.
#' They change the log-odds using main effects and an interaction:
#'
...
#' generation.
#'
#' For `ordinal = TRUE`, random normal errors are added to the linear predictor
#' (i.e. prior to computing the probability) and cut points (0.00, 0.20, 0.75,
#' and 1.00) are used to bin the probabilities into classes `"low"`, `"med"`,
#' and `"high"` (despite the function's name).
#'
...
#' `factors = TRUE`, each of the predictors is coerced into a factor. This
#' simulation can also be adapted for classification using the option `class =
#' TRUE`. In this case, the outcome is converted to be a factor by first
#' computing the logit transformation of the equation above and using uniform
#' random numbers to assign the observed class.
#'
#' A second function (`LPH07_2`) uses 20 independent Gaussians with mean zero
#' and variance 16. The functional form here is:
#'
...
#'
#' `SLC14_2` is also from Sapp et al. (2014). Two hundred independent Gaussian
#' variables are generated, each having mean zero and variance 16. The
#' functional form is
#'
...
#'   used when `ordinal = FALSE`. See Details below.
