...
#'
#' @return `bag` produces an object of class `bag` with elements:
#'
#' * `fits`: a list with two sub-objects: the `fit` object has the actual model
#'           fit for that bagged samples and the `vars` object is either
#'           `NULL` or a vector of integers corresponding to which predictors
#'           were sampled for that model
...
#' \dontrun{
#' if (caret:::has_packages("party", "MASS")) {
...
"bag.default" <-
  function(x, y, B = 10, vars = ncol(x), bagControl = NULL, ...) {
    ...
    if (is.null(bagControl)) {
      stop("Please specify 'bagControl' with the appropriate functions")
    }
    ...
    fitter <- function(index, x, y, ctrl, v, ...) {
      ...
      if (ctrl$downSample) {
        ...
        splitUp <- lapply(splitUp, sample, size = smallFreak)
      ...
      list(fit = fit, vars = subVars, oob = out)
    }
...
print.bag <- function(x, ...) {
  ...
  cat(ifelse(
    is.null(x$vars) || x$dims[2] == x$vars,
    "All variables were used in each model",
    paste(
      "Each model used",
      x$vars,
      "random",
      ifelse(x$vars == 1, "variable", "variables"),
      "predictors"
    )
  ))
  cat('\n')
  if (x$control$downSample) {
    cat(
      "Training data was down-sampled to balance the classes to",
      x$smallClass,
      "samples per class\n\n"
    )
  }
...
ldaBag <- list(
  ...
  aggregate = function(x, type = "class") {
    ...
    if (type == "class") {
      ...
      out <- as.data.frame(pooled, stringsAsFactors = TRUE)
    }
    out
  }
)
...
ctreeBag <- list(
  ...
  pred = function(object, x) {
    ...
    if (!is.null(obsLevels)) {
      ...
      out <- unlist(party::treeresponse(object, x))
    }
    out
  },
  aggregate = function(x, type = "class") {
...
nnetBag <- list(
  ...
  pred = function(object, x) {
    ...
    if (object$classification) {
      ...
      out <- predict(object, x, type = "raw")[, 1]
    }
    out
  },
  aggregate = function(x, type = "class") {
