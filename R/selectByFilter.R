...
#' set of predictor data with the appropriate subset of variables (i.e. after
#' filtering) \item`y` the current outcome data (either a numeric or factor
#' vector) \item`...` optional arguments to pass to the fit function in the
#' call to `sbf` } The function should return a model object that can be used
...
#'
#' The `score` function is used to return scores with names for each predictor
#' (such as a p-value). Inputs are: \itemize{ \item`x` the predictors for the
#' training samples. If `sbfControl()$multivariate` is `TRUE`, this will be the
#' full predictor matrix. Otherwise it is a vector for a specific predictor.
#' \item`y` the current training outcomes } When `sbfControl()$multivariate` is
#' `TRUE`, the `score` function should return a named vector where
#' `length(scores) == ncol(x)`. Otherwise, the function's output should be a
#' single value. Univariate examples are give by [anovaScores()] for
#' classification and [gamScores()] for regression and the example below.
#'
#' The `filter` function is used to return a logical vector with names for each
#' predictor (`TRUE` indicates that the prediction should be retained). Inputs
#' are: \itemize{ \item`score` the output of the `score` function \item`x` the
#' predictors for the training samples \item`y` the current training outcomes }
