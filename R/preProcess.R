...
#'
#' The Box-Cox (`method = "BoxCox"`), Yeo-Johnson (`method = "YeoJohnson"`),
#'  and exponential transformations (`method = "expoTrans"`) have been
#' "repurposed" here: they are being used to transform
...
#' the data in `x`) from the predictor values while `method = "scale"`
...
preProcess.default <- function(
    ...
    if (any(is_zv)) {
      ...
      if (verbose) {
        cat(paste(
          " ",
          length(removed),
          "zero variance predictors were removed.\n"
        ))
      }
    }
    method$zv <- NULL
  ...
  if (any(names(method) == "nzv")) {
    ...
    if (length(is_nzv) > 0) {
      removed <- colnames(x[, !(colnames(x) %in% method$ignore), drop = FALSE])[
        is_nzv
      ]
      method <- lapply(
        method,
    ...
    if (length(bad_pred) > 0) {
      ...
      if (verbose) {
        cat(paste(
          " ",
          length(removed),
          "conditionally zero variance predictors.\n"
        ))
      }
    }
  ...
  if (any(names(method) == "corr")) {
    cmat <- try(
      cor(
        x[, !(colnames(x) %in% c(method$ignore, method$remove)), drop = FALSE],
      ...
      if (length(high_corr) > 0) {
        ...
        if (verbose) {
          cat(paste(
            " ",
            length(removed),
            "highly correlated predictors were removed.\n"
          ))
        }
      }
    } else {
  ...
  if (any(names(method) == "invHyperbolicSine")) {
    if (verbose) {
      cat(" applying invHyperbolicSine\n")
    }
    for (i in method$invHyperbolicSine) {
      x[, i] <- invHyperbolicSineFunc(x[, i])
    }
  }

  if (any(names(method) == "BoxCox")) {
    bc <- group_bc(
      x[, method$BoxCox, drop = FALSE],
      fudge = fudge,
      na.remove = na.remove,
      numUnique = numUnique,
      verbose = verbose
  ...
  if (any(names(method) == "YeoJohnson")) {
    ...
    if (length(yj) > 0) {
      ...
      if (verbose) {
        cat(" all of the transformations failed\n")
      }
      yj <- NULL
    }
  } else {
  ...
  if (any(names(method) == "expoTrans")) {
    if (verbose) {
      cat(
        "Estimating exponential transformations for",
        length(method$expoTrans),
        "predictors..."
      )
    }
    if (is.data.frame(x)) {
      et <- lapply(
        x[, method$expoTrans, drop = FALSE],
        expoTrans.default,
        numUnique = numUnique
      )
    } else {
    ...
    if (length(omit_expo) > 0) {
      warning(
        paste(
          "Expo. transform induced infinite values",
  ...
  if (any(names(method) %in% c("center"))) {
    if (verbose) {
      cat("Calculating", length(method$center), "means for centering\n")
    }
    centerValue <- apply(
      x[, method$center, drop = FALSE],
      2,
      mean,
      na.rm = na.remove
    )
    x[, method$center] <- sweep(
      x[, method$center, drop = FALSE],
      2,
      centerValue,
      "-"
  ...
  if (any(names(method) == "range")) {
    if (verbose) {
      cat(
        "Calculating",
        length(method$range),
        "statistcs for scaling to a range\n"
      )
    }
    ...
    ranges <- apply(x[, method$range, drop = FALSE], 2, function(x) {
      c(min(x, na.rm = na.remove), max(x, na.rm = na.remove))
    })
    ## check for zero variance
    is_same <- apply(ranges, 2, function(x) x[1] == x[2])
    if (any(is_same)) {
      wrn <- paste(
        "No variation for for:",
        paste(names(is_same)[is_same], collapse = ", ")
      )
      warning(wrn, immediate. = TRUE)
      ranges <- ranges[, !is_same, drop = FALSE]
      method$range <- method$range[!(method$range %in% names(is_same)[is_same])]
    }
    x[, method$range] <- sweep(
      x[, method$range, drop = FALSE],
      2,
      ranges[1, ],
      "-"
    )
  ...
  if (any(names(method) == "bagImpute")) {
    if (verbose) {
      cat(
        "Computing bagging models for",
        length(method$bagImpute),
        "predictors..."
      )
    }
  ...
  if (any(names(method) == "medianImpute")) {
    if (verbose) {
      cat("Computing medians for", length(method$medianImpute), "predictors...")
    }
    medianValue <- apply(
      x[, method$medianImpute, drop = FALSE],
      2,
      median,
      na.rm = TRUE
    )
  ...
  if (any(names(method) == "pca")) {
    if (verbose) {
      cat("Computing PCA loadings for", length(method$pca), "predictors\n")
    }
    ...
    if (is.null(pcaComp)) {
      ...
      numComp <- min(pcaComp, ncol(tmp$rotation))
    }
  ...
  if (any(names(method) == "ica")) {
    ...
    if (verbose) {
      cat("Computing ICA loadings for", length(method$ica), "predictors\n")
    }
    requireNamespaceQuietStop("fastICA")
    tmp <- fastICA::fastICA(x[, method$ica, drop = FALSE], ...)
    ica <- list(row.norm = row.norm, K = tmp$K, W = tmp$W)
  } else {
    ica <- NULL
  }

  out <- list(
    dim = c(nrow(x), length(unique(unlist(method)))),
    bc = bc,
    yj = yj,
    et = et,
    ...
    data = if (any(names(method) == "knnImpute")) {
      x[complete.cases(x), method$knnImpute, drop = FALSE]
    } else {
      NULL
    },
...
predict.preProcess <- function(object, newdata, ...) {
  ...
  if (!is.null(object$method$remove)) {
    if (length(object$method$remove) > 0) {
      newdata <- newdata[,
        !(colnames(newdata) %in% object$method$remove),
        drop = FALSE
      ]
    }
    if (ncol(newdata) == 0) {
  ...
  if (any(names(object$method) == "range")) {
    ...
    newdata[, object$method$range] <-
      sweep(
        newdata[, object$method$range, drop = FALSE],
        2,
        object$ranges[1, ],
        "-"
      )
  ...
  if (any(names(object$method) == "knnImpute") && any(!cc)) {
    ...
    if (class(newdata)[1] == class(hasMiss)[1]) {
      ...
      if (is.data.frame(newdata)) {
        newdata[!cc, object$method$knnImpute] <- as.data.frame(
          hasMiss,
          stringsAsFactors = TRUE
        )
      } else {
        newdata[!cc, object$method$knnImpute] <- as.matrix(hasMiss)
      }
    }
  }
  ...
  if (any(names(object$method) == "bagImpute") && any(!cc)) {
    ...
    if (!is.data.frame(hasMiss)) {
      hasMiss <- as.data.frame(hasMiss, stringsAsFactors = TRUE)
    }
    for (i in seq(along.with = missingVars)) {
      preds <- predict(
        object$bagImp[[missingVars[i]]]$model,
        hasMiss[, !colnames(hasMiss) %in% missingVars[i], drop = FALSE]
      )

      hasMiss[
        is.na(hasMiss[, missingVars[i]]),
        missingVars[i]
      ] <- preds[is.na(hasMiss[, missingVars[i]])]
    }
    if (class(newdata)[1] == class(hasMiss)[1]) {
      newdata[!cc, ] <- hasMiss
    } else {
  ...
  if (any(names(object$method) == "pca")) {
    ...
    if (is.data.frame(newdata)) {
      pca_cols <- as.data.frame(pca_cols, stringsAsFactors = TRUE)
    }
    ...
    if (length(discard) > 0) {
      newdata <- newdata[, !(colnames(newdata) %in% discard), drop = FALSE]
    }
  }

  if (any(names(object$method) == "ica")) {
    ica_cols <- newdata[, object$method$ica, drop = FALSE]
    if (!is.matrix(ica_cols)) {
      ica_cols <- as.matrix(ica_cols)
    }
  ...
  if (
    ...
    newdata[, ss_col_names] <- spatialSign(newdata[,
      ss_col_names,
      drop = FALSE
    ])
  }
...
print.preProcess <- function(x, ...) {
  ...
  pp <- gsub(
    "range",
    paste0("re-scaling to [", rangeBounds$lower, ", ", rangeBounds$upper, "]"),
    pp
  )
  ...
  if (any(names(x$method) == "pca")) {
    if (is.null(x$pcaComp)) {
      cat(
        "PCA needed",
        x$numComp,
        ifelse(x$numComp > 1, "components", "component"),
        "to capture",
        round(x$thresh * 100, 2),
      ...
      cat(
        "PCA used",
        x$pcaComp,
        ifelse(x$pcaComp > 1, "components", "component"),
        "as specified"
      )
    }
...
nnimp <- function(new, old, k, foo) {
  ...
  nn <- RANN::nn2(
    old[, non_missing_cols, drop = FALSE],
...
bagImp <- function(var, x, B = 10) {
  ...
  list(var = var, model = trim_code(mod))
}
...
pre_process_options <- function(opts, vars) {
  ...
  if (!all(names(opts) %in% ppMethods)) {
    ...
    stop(
      (paste(
        "These pre-processing methods are unknown:",
  ...
  if (!all(all_op_vars %in% names(vars))) {
    ...
    stop(
      (paste(
        "These fields are not in the data:",
        paste("'", others, "'", sep = "", collapse = ", ")
      ))
    )
  ...
  if ("pca" %in% methods && length(opts[["pca"]]) == 1) {
    warning(
      paste(
        "PCA is a group transformation and only a single predictor",
  ...
  if ("ica" %in% methods && length(opts[["ica"]]) == 1) {
    warning(
      paste(
        "ICA is a group transformation and only a single predictor",
        "is listed. This method is eliminated."
      ),
      immediate. = TRUE
    )
  ...
  if (
    ...
    warning(
      paste(
        "Spatial sign is a group transformation and only a single predictor",
        "is listed. This method is eliminated."
      ),
      immediate. = TRUE
    )
  ...
  if (all(c("pca", "ica") %in% methods)) {
    ...
    if (length(pca_and_ica) > 0) {
      warning(
        paste(
          "fastICA automatically uncorrelates the data using PCA.",
  ...
  if (sum(c("knnImpute", "bagImpute", "medianImpute") %in% methods) > 1) {
    ...
    if (any(imp_table > 1)) {
      ...
      stop(
        (paste(
          "Please pick a single imputation method for:",
  ...
  if ("ignore" %in% names(opts)) {
    opts$ignore <- unique(c(not_num_vars, opts$ignore))
  } else {
...
check_for_wildcards <- function(opts, verbose = TRUE) {
  ...
  if (any(pc_wc)) {
    ...
    if (verbose) {
      cat("PCA wildcards found for:", paste(pc_wc, sep = "", collapse = ", "))
    }
    if (any(pc_wc %in% other_methods)) {
      bad_ops <- pc_wc[pc_wc %in% other_methods]
      if (verbose) {
        cat(
          " ...but should not be in methods:",
  ...
  pc_wc <- unlist(lapply(opts, function(x) any(x == "_PC_")))
  ic_wc <- unlist(lapply(opts, function(x) any(x == "_IC_")))
  wc_list <- list(PCA = names(pc_wc)[pc_wc], ICA = names(ic_wc)[ic_wc])
  opts <- lapply(opts, function(x) x[!(x %in% c("_PC_", "_IC_"))])
  list(opts = opts, wildcards = wc_list)
}


group_bc <- function(x, outcome = NULL, fudge, na.remove, numUnique, verbose) {
  if (verbose) {
    cat("Estimating Box-Cox transformations for", ncol(x), "predictors...\n")
  }
  if (is.matrix(x)) {
    bc <- apply(
      x,
  ...
  if (any(is.na(lambdas))) {
    ...
    if (verbose) {
      cat("Box-Cox failed for:", paste(bad_lambda, sep = "", collapse = ", "))
    }
...
convert_method <- function(x) {
  new_method <- list()
  if ("center" %in% x$method) {
    new_method$center <- names(x$mean)
  }
  if ("scale" %in% x$method) {
    new_method$scale <- names(x$std)
  }
  if ("YeoJohnson" %in% x$method) {
    new_method$YeoJohnson <- names(x$yj)
  }
  if ("expoTrans" %in% x$method) {
    new_method$expoTrans <- names(x$et)
  }
  if ("BoxCox" %in% x$method) {
    new_method$BoxCox <- names(x$bc)
  }
  if ("knnImpute" %in% x$method) {
    new_method$knnImpute <- names(x$mean)
  }
  if ("bagImpute" %in% x$method) {
    new_method$bagImpute <- names(x$bagImp)
  }
  if ("medianImpute" %in% x$method) {
    new_method$medianImpute <- names(x$median)
  }
  if ("pca" %in% x$method) {
    new_method$pca <- names(x$mean)
  }
  if ("ica" %in% x$method) {
    new_method$ica <- names(x$mean)
  }
  if ("spatialSign" %in% x$method) {
