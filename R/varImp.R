...
#' Otherwise:
#'
#' **Linear Models**: the absolute value of the t-statistic for each model
#' parameter is used.
#'
...
#' wrappers around the importance functions from the \pkg{randomForest} and
#' \pkg{party} packages, respectively.
#'
#' **Partial Least Squares**: the variable importance measure here is based on
#' weighted sums of the absolute regression coefficients. The weights are a
...
#'
#' **Boosted Trees**: `varImp.gbm` is a wrapper around the function from that
#' package (see the \pkg{gbm} package vignette)
#'
...
#' function for these calculations. Currently, the `varImp` is a wrapper to the
#' [earth::evimp()] function in the `earth` package. There are three statistics
#' that can be used to estimate variable importance in MARS models. Using
#' `varImp(object, value = "gcv")` tracks the reduction in the generalized
#' cross-validation statistic as terms are added. However, there are some cases
#' when terms are retained in the model that result in an increase in GCV.
#' Negative variable importance values for MARS are set to zero.
#' Alternatively, using `varImp(object, value = "rss")` monitors the change in
#' the residual sums of squares (RSS) as terms are added, which will never be
#' negative.  Also, the option `varImp(object, value =" nsubsets")`, which
#' counts the number of subsets where the variable is used (in the final,
#' pruned model).
#'
#' **Nearest shrunken centroids**: The difference between the class centroids
#' and the overall centroid is used to measure the variable influence (see
#' `pamr.predict`). The larger the difference between the class centroid and
#' the overall center of the data, the larger the separation between the
#' classes. The training set predictions must be supplied when an object of
#' class `pamrtrained` is given to `varImp`.
#'
#' **Cubist**: The Cubist output contains variable usage statistics. It gives
#' the percentage of times where each variable was used in a condition and/or a
#' linear model. Note that this output will probably be inconsistent with the
#' rules shown in the output from [Cubist::summary.cubist()]. At each split of
#' the tree, Cubist saves a linear model (after feature selection) that is
#' allowed to have terms for each variable used in the current split or any
...
#' model.
#'
#' **PART** and **JRip**: For these rule-based models, the importance for a
#' predictor is simply the number of rules that involve the predictor.
#'
#' **C5.0**: C5.0 measures predictor importance by determining the percentage
...
#'
#' **Neural Networks**: The method used here is based on Gevrey et al (2003),
#' which uses combinations of the absolute values of the weights. For
#' classification models, the class-specific importances will be the same.
#'
#' **Recursive Feature Elimination**: Variable importance is computed using the
#' ranking method used for feature selection. For the final subset size, the
#' importances for the models across all resamples are averaged to compute an
#' overall value.
#'
#' **Feature Selection via Univariate Filters**, the percentage of resamples
...
#'   importance?  This is only used for some models (lm, pls, rf, rpart, gbm,
#'   pam and mars)
#' @param nonpara should nonparametric methods be used to assess the
#'   relationship between the features and response (only used with `useModel =
#'   FALSE` and only passed to `filterVarImp`).
#' @param scale should the importance values be scaled to 0 and 100?
#' @param \dots parameters to pass to the specific `varImp` methods
#' @param numTrees the number of iterations (trees) to use in a boosted tree
#'   model
#' @param threshold the shrinkage threshold (`pamr` models only)
#' @param data the training set predictors (`pamr` models only)
...
#'   [pls::mvrVal()]
#' @param cuts the number of rule sets to use in the model (for `partDSA` only)
...
#' @return A data frame with class `c("varImp.train", "data.frame")` for
