...
#' - `last` similar to `first`, but `TRUE` when the last model is fit
#'   with the final subset size and predictors.
...
#' most important to the least important. Inputs are:
#' - `object` the model generated by the `fit` function
#' - `x` the current set of predictor set for the training samples
#' - `y` the current training outcomes
#'
#' The function should return a data frame with a column called `var` that
#' has the current variable names. The first row should be the most important
#' predictor etc. Other columns can be included in the output and will be
#' returned in the final `rfe` object.
#'
...
#' - `x` a matrix with columns for the performance metrics and the
#'   number of variables, called "`Variables`"
#' - `metric` a character string of the performance measure to optimize
#'   (e.g. "RMSE", "Rsquared", "Accuracy" or "Kappa")
#' - `maximize` a single logical for whether the metric should be
#'   maximized
#'
#' This function should return an integer corresponding to the optimal
#' subset size. \pkg{caret} comes with two examples functions for this
#' purpose: [pickSizeBest()] and [pickSizeTolerance()].
#'
#' After the optimal subset size is determined, the `selectVar` function
#' will be used to calculate the best rankings for each variable across all
#' the resampling iterations. Inputs for the function are:
#' - `y` a list of variables importance for each resampling iteration
#'   and each subset size (generated by the user-defined `rank`
#'   function). In the example, each of the cross-validation groups the
#'   output of the `rank` function is saved for each of the subset sizes
#'   (including the original subset). If the rankings are not recomputed
#'   at each iteration, the values will be the same within each
#'   cross-validation iteration.
#' - `size` the integer returned by the `selectSize` function
#'
#' This function should return a character string of predictor names (of
#' length `size`) in the order of most important to least important
#'
