findCorrelation_fast <- function(x, cutoff = 0.90, verbose = FALSE) {
  ...
  if (verbose) {
    colsFlagged <- pmin(
      ifelse(colsToDiscard, colsToCheck, NA),
      ifelse(rowsToDiscard, rowsToCheck, NA),
      na.rm = TRUE
    )
...
#' @return A vector of indices denoting the columns to remove (when
#'   `names = TRUE`) otherwise a vector of column names. If no correlations
#'   meet the criteria, `integer(0)` is returned.
