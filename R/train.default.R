...
#'
#' `train` can be used to tune models by picking the complexity parameters that
#' are associated with the optimal resampling statistics. For particular model,
#' a grid of parameters (if any) is created and the model is trained on
...
#'
#' The predictors in `x` can be most any object as long as the underlying model
#' fit function can deal with the object class. The function was designed to
#' work with simple matrices and data frame inputs, so some functionality may
#' not work (e.g. pre-processing). When using string kernels, the vector of
...
#' <http://topepo.github.io/caret/using-recipes-with-train.html>. Note that
#' case weights can be passed into `train` using a role of `"case weight"` for
#' a single variable. Also, if there are non-predictor columns that should be
#' used when determining the model's performance metrics, the role of
#' `"performance var"` can be used with multiple columns and these will be made
#' available during resampling to the `summaryFunction` function.
#'
#' @aliases train train.default train.formula
#' @param x For the default method, `x` is an object where samples are in rows
#'   and features are in columns. This could be a simple matrix, data frame or
#'   other type (e.g. sparse matrix) but must have column names (see Details
#'   below). Preprocessing using the `preProcess` argument only supports
#'   matrices or data frames. When using the recipe method, `x` should be an
#'   unprepared [recipes::recipe()] object that describes the model terms (i.e.
#'   outcome, predictors, etc.) as well as any pre-processing that should be
#'   done to the data. This is an alternative approach to specifying the model.
#'   Note that, when using the recipe method, any arguments passed to
#'   `preProcess` will be ignored. See the links and example below for more
...
#' @param data Data frame from which variables specified in `formula` or
...
#'   to use. Possible values are found using `names(getModelInfo())`. See
#'   <http://topepo.github.io/caret/train-models-by-tag.html>. A list of
#'   functions can also be passed for a custom model function. See
#'   <http://topepo.github.io/caret/using-your-own-model-in-train.html> for
#'   details.
#' @param \dots Arguments passed to the classification or regression routine
#'   (such as [randomForest::randomForest()]). Errors will occur if values for
...
#'   pre-processing. See [preProcess()] and [trainControl()] on the procedures
#'   and how to adjust them. Pre-processing code is only designed to work when
#'   `x` is a simple matrix or data frame.
...
#'   by the `summaryFunction` is used. (NOTE: If given, this argument must be
#'   named.)
...
#'   [trainControl()] and
#'   <http://topepo.github.io/caret/using-your-own-model-in-train.html>. (NOTE:
#'   If given, this argument must be named.)
#' @param tuneGrid A data frame with possible tuning values. The columns are
#'   named the same as the tuning parameters. Use [getModelInfo()] to get a
#'   list of tuning parameters for each model or see
#'   <http://topepo.github.io/caret/available-models.html>. (NOTE: If given,
...
#'   for each tuning parameters that should be generated by [train()]. If
#'   [trainControl()] has the option `search = "random"`, this is the maximum
#'   number of tuning parameter combinations that will be generated by the
#'   random search. (NOTE: If given, this argument must be named.)
#' @return A list is returned of class `train` containing:
#'
...
#'               requested, this will be `NULL`. The `returnResamp` argument of
#'               [trainControl()] controls how much of the resampled results
#'               are saved.
...
#' * `times`: A list of execution times: `everything` is for the entire call to
#'            `train`, `final` for the final model fit and, optionally,
#'            `prediction` for the time to predict new samples (see
#'            [trainControl()])
#' @author Max Kuhn (the guts of `train.formula` were based on Ripley's
#'   `nnet.formula`)
#' @seealso [models()], [trainControl()], [update.train()], [modelLookup()],
#'   [createFolds()], [recipes::recipe()]
#' @references <http://topepo.github.io/caret/>
#'
...
train.default <- function(
  ...
  maximize = ifelse(
    metric %in% c("RMSE", "logLoss", "MAE", "logLoss"),
    FALSE,
    TRUE
  ),
  ...
  if (is.list(method)) {
    ...
    if (length(models) == 0) {
      stop(
        paste("Model", method, "is not in caret's built-in library"),
        call. = FALSE
      )
    }
  }
  ...
  for (i in seq(along.with = models$library)) {
    do.call("requireNamespaceQuietStop", list(package = models$library[i]))
  }
  ...
  if (modelType == "Classification") {
    ...
    if (metric %in% c("RMSE", "Rsquared")) {
      stop(
        paste("Metric", metric, "not applicable for classification models"),
        call. = FALSE
      )
    }
    ...
    if (metric %in% c("Accuracy", "Kappa")) {
      stop(
        paste("Metric", metric, "not applicable for regression models"),
        call. = FALSE
      )
    }
    classLevels <- NA
  ...
  if (trControl$method != "none") {
    ...
    if (is.null(trControl$seeds) || all(is.na(trControl$seeds))) {
      seeds <- sample.int(
        n = 1000000L,
        size = num_rs * nrow(trainInfo$loop) + 1L
      )
      seeds <- lapply(
        seq(from = 1L, to = length(seeds), by = nrow(trainInfo$loop)),
        function(x) {
        ...
        if (badSeed) {
          stop(
            paste(
              ...
              nrow(trainInfo$loop),
              "and the last list element having at least a",
              "single integer"
            ),
            call. = FALSE
          )
    ...
    if (trControl$method == "oob") {
      ...
      testSummary <- evalSummaryFunction(
        y,
        wts = weights,
        ctrl = trControl,
        lev = classLevels,
    ...
    if (trControl$method == "oob") {
      ...
      perfNames <- perfNames[
        !(perfNames %in% as.character(models$parameters$parameter))
      ]
      if (!(metric %in% perfNames)) {
        oldMetric <- metric
        metric <- perfNames[1]
    ...
    if (!(trControl$method %in% c("LOOCV", "oob"))) {
      if (
        modelType == "Classification" &&
          length(grep("^\\cell", colnames(resampleResults))) > 0
      ) {
        resampledCM <- resampleResults[,
          !(names(resampleResults) %in% perfNames)
        ]
        resampleResults <- resampleResults[,
          -grep("^\\cell", colnames(resampleResults))
        ]
    ...
    if (selectClass == "function") {
      ...
      if (trControl$selectionFunction == "oneSE") {
        bestIter <- oneSE(perf_check, metric, length(trControl$index), maximize)
      } else {
        bestIter <- do.call(
    ...
    bestTune <- tuneGrid
    performance <- evalSummaryFunction(
      y,
      wts = weights,
      ctrl = trControl,
      lev = classLevels,
  ...
  orderList <- list()
  for (i in seq(along.with = paramNames)) {
  ...
  if (trControl$verboseIter) {
    bestText <- paste(
      paste(names(bestTune), "=", format(bestTune, digits = 3)),
      collapse = ", "
    )
...
train.formula <- function(
  ...
  m <- eval.parent(m)
  if (nrow(m) < 1) {
    stop("Every row has at least one missing value were found", call. = FALSE)
  }
  Terms <- attr(m, "terms")
...
train.recipe <- function(
  ...
  # the recipe
  y_orig_val <- trained_rec$var_info$variable[
    trained_rec$var_info$role == "outcome"
  ]
  ...
  if (trControl$method != "none") {
    ...
    if (grepl("adapt", trControl$method)) {
      perf_check <- subset(
        performance,
        Num_Resamples == max(performance$Num_Resamples)
      )
    } else {
      perf_check <- performance
    }
...
residuals.train <- function(object, ...) {
  if (object$modelType != "Regression") {
    stop("train() only produces residuals on numeric outcomes", call. = FALSE)
  }
  resid <- residuals(object$finalModel, ...)
  if (is.null(resid)) {
    if (!is.null(object$trainingData)) {
      resid <- object$trainingData$.outcome -
        predict(
          object,
          object$trainingData[,
            names(object$trainingData) != ".outcome",
            drop = FALSE
          ]
        )
    } else {
...
fitted.train <- function(object, ...) {
  ...
  if (is.null(prd)) {
    if (!is.null(object$trainingData)) {
      ...
      stop(
        "The training data must be saved to produce fitted values",
        call. = FALSE
      )
    }
  }
