...
#' splits the data into `k` groups while `createTimeSlices` creates
...
#' `y` when `y` is a factor in an attempt to balance the class distributions
#' within the splits.
#'
...
#' `createDataPartition`, the number of percentiles is set via the `groups`
#' argument. For `createFolds` and `createMultiFolds`, the number of groups is
#' set dynamically based on the sample size and `k`.  For smaller samples
#' sizes, these two functions may not do stratified splitting and, at most,
#' will split the data into quartiles.
#'
#' Also, for `createDataPartition`, very small class sizes (<= 3) the classes
...
#' techniques that move the training and test sets in time. `createTimeSlices`
#' can create the indices for this type of splitting.
#'
#' For Group k-fold cross-validation, the data are split such that no group is
#' contained in both the modeling and holdout sets. One or more group could be
#' left out, depending on the value of `k`.
#'
#' @aliases createDataPartition createResample createFolds createMultiFolds createTimeSlices groupKFold
#' @param y a vector of outcomes. For `createTimeSlices`, these should be in
#'   chronological order.
...
#'   columns.
#' @param groups for numeric `y`, the number of breaks in the quantiles (see
#'   below)
...
#' @author Max Kuhn, `createTimeSlices` by Tony Cooper
#' @references <http://topepo.github.io/caret/data-splitting.html>
#'
